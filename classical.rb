...
class Tetris
  ...
  def buttons
    ...
    new_game.place(35, 110, 60, 10)
    
    quit = TetrisButton.new('QUIT (Q)', 'lightcoral'){exitProgram}
    quit.place(35, 110, 320, 10)
    
    move_left = TetrisButton.new('LEFT (A)', 'lightgreen'){@board.move_left}
    move_left.place(35, 90, 105, 630)
    
    move_right = TetrisButton.new('RIGHT (D)', 'lightgreen'){@board.move_right}
    move_right.place(35, 90, 305, 630)
    
    rotate_clock = TetrisButton.new('↻ (W)', 'lightgreen'){@board.rotate_clockwise}
    rotate_clock.place(35, 90, 205, 590)

    rotate_counter = TetrisButton.new('↺ (S)', 'lightgreen'){
      @board.rotate_counter_clockwise}
    rotate_counter.place(35, 90, 205, 670)
    
    drop = TetrisButton.new('DROP (SPC)', 'lightgreen'){@board.drop_all_the_way}
    drop.place(35, 90, 205, 630)

    label = TetrisLabel.new(@root) do
      text 'Current Score: '   
      background 'azure'
    end
    label.place(35, 100, 175, 60)
    @score = TetrisLabel.new(@root) do
      background 'azure'
    end
    @score.text(@board.score)
    @score.place(35, 50, 275, 60)    
  end
